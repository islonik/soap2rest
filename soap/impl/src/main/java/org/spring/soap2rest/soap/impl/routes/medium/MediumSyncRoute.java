 ...
 */
@Component
public class MediumSyncRoute {

    private static final Logger log = LoggerFactory.getLogger(MediumSyncRoute.class);

    @Autowired
    private SyncLogic syncLogic;

    @ServiceActivator(inputChannel = RouteServices.MEDIUM_SYNC_ID)
    public ServiceOrderStatus processOrder(Service service) {
        log.info("MediumSyncRoute");
        return syncLogic.medium(service);
    }
